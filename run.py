...
def run_case(q_dir, meta, case, harness_file):
    """Run one test case in a subprocess. Returns (status, detail, elapsed)."""
    start = time.monotonic()
    try:
        proc = subprocess.run(
            [sys.executable, str(harness_file), str(q_dir / "code.py"),
...
def test_question(module, slug, q_dir, only_case=None):
    meta = load_meta(q_dir)
    cases = json.loads((q_dir / "tests" / "cases.json").read_text())
    if not (q_dir / "code.py").exists():
        print(f"{YELLOW}∅ {slug}: no code.py yet{RESET}")
        return None

    harness_file = ROOT / ".harness.py"
    harness_file.write_text(HARNESS)

    print(f"{BOLD}{module}/{slug}{RESET} "
          f"{DIM}({meta['difficulty']}, {meta['time_limit_seconds']}s limit){RESET}")
    passed = 0
    selected = [(i, c) for i, c in enumerate(cases)
                if only_case is None or i == only_case]
    for i, case in selected:
        status, detail, elapsed = run_case(q_dir, meta, case, harness_file)
        if status == "PASS":
            passed += 1
...
def cmd_test(args):
    ...
    ok = test_question(module, slug, q_dir, only_case=args.case)
    if ok:
        (q_dir / ".solved").touch()
...
TEMPLATE_CASES = [{"input": {}, "expected": None}]
...
def cmd_new(args):
    ...
    (q_dir / "code.py").write_text(TEMPLATE_CODE)
